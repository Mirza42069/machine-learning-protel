...
while True:
    ...
    cv2.putText(frame_resized, "Adjust Lighting. Press 'C' to Continue.", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    cv2.putText(frame_resized, "Press 'Q' to Quit.", (20, 80),
    ...
    if key == ord('c'):
        print("--- Calibration Complete. Starting Main Detection Loop. ---")
        cv2.destroyWindow("Camera Calibration")
        break
    if key == ord('q'):
        print("Quitting program during calibration.")
        tello.streamoff()
        tello.end()
